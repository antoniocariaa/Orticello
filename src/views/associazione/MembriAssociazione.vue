...
const fetchUsers = async () => {
    try {
        ...
        const response = await api.get('/utenti/associazione')
        users.value = response
    } catch (err) {
        console.error('Error fetching users:', err)
        error.value = err.message || t('members_asso.add_error')
    } finally {
        loading.value = false
    }
}
...
const removeAssociazioneRole = async () => {
    if (!userToRemove.value) return
    try {
        removing.value = true
        await api.put(`/utenti/removeAssociazioneRole/${userToRemove.value._id}`)
        users.value = users.value.filter(u => u._id !== userToRemove.value._id)
        userToRemove.value = null
        document.getElementById('remove-role-modal').close()
    } catch (err) {
        console.error('Error removing role:', err)
        error.value = t('members_asso.remove_error')
    } finally {
...
const addAssociazioneMember = async () => {
    ...
    try {
        ...
        addError.value = err.message || t('members_asso.add_error')
    } finally {
        adding.value = false
    }
}
...
<template>
    <div class="container mx-auto p-4 md:p-8">
                ...
                <div class="card-body">
                    <div class="flex items-start justify-between">
                        ...
                        <button
                            ...
                            :title="$t('members_asso.remove_role')"
        ...
        <div v-if="!loading && !error && users.length === 0" class="text-center py-12 text-base-content/60">
            ...
            <p>{{ $t('members_asso.no_members') }}</p>
        </div>

        <!-- Remove Role Confirmation Modal -->
        <dialog id="remove-role-modal" class="modal">
            <div class="modal-box">
                <h3 class="font-bold text-lg">{{ $t('members_asso.remove_role_title') }}</h3>
                <p class="py-4">
                    {{ $t('members_asso.remove_role_confirm', { name: userToRemove ? `${userToRemove.nome} ${userToRemove.cognome}` : '' }) }}
                </p>
        ...
        <dialog id="add-member-modal" class="modal">
            <div class="modal-box">
                <h3 class="font-bold text-lg">{{ $t('members_asso.add_member_title') }}</h3>
                
                <div class="form-control mt-4">
                    <label class="label">
                        <span class="label-text">{{ $t('members_asso.add_member_email') }}</span>
                    </label>
                    <input
                        type="email"
                        v-model="addEmail"
                        :placeholder="$t('members_asso.add_member_email_placeholder')"
                        class="input input-bordered w-full"
                        @keyup.enter="addAssociazioneMember"
                    />
                </div>

                <div class="form-control mt-4">
                    <label class="label cursor-pointer">
                        <span class="label-text">{{ $t('members_asso.add_member_admin') }}</span>
